...
export function addScriptMark(marks, options) {
    return marks.append({
        script: getScriptMarkDef(options)
    })
}

export function toggleSuperScript(view) {
    return applyScript("sup", view)
}

export function toggleSubScript(view) {
    ...
    if (to > from) {
        view.state.doc.nodesBetween(from, to, function (node) {
            if (
                ...
                markType.isInSet(node.marks).attrs.type === type
            ) {
                found = true;
            }
            return !found;
