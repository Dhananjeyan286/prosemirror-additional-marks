var ProsemirrorAdditionalMarksDemo = (function (exports) {
  'use strict';
  ...
  */
  ...
  function replaceThreeWay$1($from, $start, $end, $to, depth) {
      let openStart = $from.depth > depth && joinable$3($from, $start, depth + 1);
      let openEnd = $to.depth > depth && joinable$3($end, $to, depth + 1);
      let content = [];
  ...
  function replaceTwoWay$1($from, $to, depth) {
      ...
      addRange$1(null, $from, depth, content);
      if ($from.depth > depth) {
          let type = joinable$3($from, $to, depth + 1);
  ...
  */
  function canJoin$1(doc, pos) {
      let $pos = doc.resolve(pos), index = $pos.index();
      return joinable$2($pos.nodeBefore, $pos.nodeAfter) &&
          $pos.parent.canReplace(index, index + 1);
  }
  function joinable$2(a, b) {
  ...
  */
  function joinPoint(doc, pos, dir = -1) {
      ...
      for (let d = $pos.depth;; d--) {
          ...
          if (before && !before.isTextblock && joinable$2(before, after) &&
  ...
  */
  ...
  const mac$5 = ios || (nav ? /Mac/.test(nav.platform) : false);
  ...
  const selectNodeModifier = mac$5 ? "metaKey" : "ctrlKey";
  ...
  */
  ...
  const dragCopyModifier = mac$5 ? "altKey" : "ctrlKey";
      ...
      return function (state, dispatch) {
          ...
          if ($from.parent.content.size == 0 && $from.node(-1).childCount == $from.indexAfter(-1)) {
              ...
              if (dispatch) {
                  ...
                  let tr = state.tr.replace(start, $from.after(-depthAfter), new Slice$1(wrap, 4 - depthBefore, 0));
                  let sel = -1;
                  ...
                  if (sel > -1)
                      tr.setSelection(Selection$1.near(tr.doc.resolve(sel)));
  ...
  */
  ...
  function liftToOuterList(state, dispatch, itemType, range) {
      ...
      const target = liftTarget$1(range);
      if (target == null)
          return false;
      tr.lift(range, target);
      let after = tr.mapping.map(end, -1) - 1;
      if (canJoin$1(tr.doc, after))
          tr.join(after);
      dispatch(tr.scrollIntoView());
      return true;
  }
  ...
  */
  ...
  function addLinkMark(marks, options) {
    return marks.append({
      link: getLinkMarkDef()
    });
  }
  ...
  function getAllLinkRanges(view) {
    // all leaf nodes
    var allTextNodes = [];
    view.state.doc.content.nodesBetween(0, view.state.doc.content.size, function (node, pos) {
      if (node.isLeaf) {
        allTextNodes.push({
          node: node,
          pos: pos
  ...
  function joinLinks(links, view) {
    ...
    var joinedLinks = links.reduce(function (joinedLinks, link) {
      ...
      } else if (link.start - previousLink.end === 2 &&
      // 2 - gap between two paras
      !view.state.doc.resolve(previousLink.end).nodeAfter && previousLink.href === link.href) {
  ...
  function addLink(options, view, link) {
  ...
  function addLinkWithText(options, view, link, text) {
    var regurl = options.regex || defaultLinkRegex;
    var isValidUrl = regurl.test(link);
  ...
  */
  ...
  var Fitter = /*#__PURE__*/function () {
    function Fitter($from, $to, unplaced) {
      ...
      for (var _i2 = $from.depth; _i2 > 0; _i2--) this.placed = Fragment.from($from.node(_i2).copy(this.placed));
    }
  ...
  var TextSelection = /*#__PURE__*/function (_Selection) {
    ...
    */
    ...
    return _createClass(TextSelection, [{
      ...
      value: function replace(tr) {
        var content = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : Slice.empty;
        _superPropGet(TextSelection, "replace", this)([tr, content]);
    ...
    return _createClass(AllSelection, [{
      ...
      value: function replace(tr) {
        ...
        if (content == Slice.empty) {
          ...
          _superPropGet(AllSelection, "replace", this)([tr, content]);
        }
      }
    }, {
      key: "toJSON",
  ...
  */
  var deleteSelection$1 = function deleteSelection(state, dispatch) {
  ...
  */
  var joinBackward$1 = function joinBackward(state, dispatch, view) {
    var $cursor = atBlockStart$1(state, view);
    ...
    if (deleteBarrier$1(state, $cut, dispatch, -1)) return true;
  ...
  function textblockAt$1(node, side) {
  ...
  */
  var selectNodeBackward$1 = function selectNodeBackward(state, dispatch, view) {
  ...
  function atBlockEnd$1(state, view) {
    var $cursor = state.selection.$cursor;
  ...
  */
  var joinForward$1 = function joinForward(state, dispatch, view) {
    var $cursor = atBlockEnd$1(state, view);
    if (!$cursor) return false;
    var $cut = findCutAfter$1($cursor);
  ...
  */
  var selectNodeForward$1 = function selectNodeForward(state, dispatch, view) {
  ...
  */
  var newlineInCode$1 = function newlineInCode(state, dispatch) {
  ...
  */
  var exitCode$1 = function exitCode(state, dispatch) {
  ...
  */
  var createParagraphNear$1 = function createParagraphNear(state, dispatch) {
  ...
  block.
  */
  var liftEmptyBlock$1 = function liftEmptyBlock(state, dispatch) {
    var $cursor = state.selection.$cursor;
  ...
  */
  function splitBlockAs$1(splitNode) {
    return function (state, dispatch) {
  ...
  */
  var splitBlock$1 = splitBlockAs$1();
  /**
  Select the whole document.
  */
  var selectAll$1 = function selectAll(state, dispatch) {
  ...
  function deleteBarrier$1(state, $cut, dispatch, dir) {
    var before = $cut.nodeBefore,
      after = $cut.nodeAfter,
      conn,
      match;
    var isolated = before.type.spec.isolating || after.type.spec.isolating;
    if (!isolated && joinMaybeClear$1(state, $cut, dispatch)) return true;
  ...
  function markApplies$2(doc, ranges, type, enterAtoms) {
  ...
  document.
  */
  function toggleMark$1(markType) {
    var attrs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    ...
    return function (state, dispatch) {
      ...
      if (empty && !$cursor || !markApplies$2(state.doc, ranges, markType, enterAtoms)) return false;
      if (dispatch) {
        if ($cursor) {
  ...
  */
  ...
  var backspace$1 = chainCommands$1(deleteSelection$1, joinBackward$1, selectNodeBackward$1);
  var del$1 = chainCommands$1(deleteSelection$1, joinForward$1, selectNodeForward$1);
  /**
  ...
  */
  ({
    "Enter": chainCommands$1(newlineInCode$1, createParagraphNear$1, liftEmptyBlock$1, splitBlock$1),
  ...
  function addBoldMark(marks, options) {
    return marks.append({
      bold: getBoldMarkDef()
    });
  }
  function toggleBold(view) {
    return toggleMark$1(view.state.schema.marks.bold)(view.state, view.dispatch);
  }
  ...
  function addItalicsMark(marks, options) {
    return marks.append({
      italics: getItalicsMarkDef()
    });
  }
  function toggleItalics(view) {
    return toggleMark$1(view.state.schema.marks.italics)(view.state, view.dispatch);
  }
  ...
  function addUnderlineMark(marks, options) {
    return marks.append({
      underline: getUnderlineMarkDef()
    });
  }
  function toggleUnderline(view) {
    return toggleMark$1(view.state.schema.marks.underline)(view.state, view.dispatch);
  }
  ...
  function addStrikethroughMark(marks, options) {
    return marks.append({
      strikethrough: getStrikethroughMarkDef()
    });
  }
  function toggleStrikethrough(view) {
    return toggleMark$1(view.state.schema.marks.strikethrough)(view.state, view.dispatch);
  }

  function markApplies$1(doc, ranges, type) {
    var rangeAllowsMark = function rangeAllowsMark(i) {
      var ref = ranges[i];
      var $from = ref.$from;
      var $to = ref.$to;
  ...
  function setFontMark(markType, value, view) {
    ...
    if (empty && !$cursor || !markApplies$1(view.state.doc, ranges, markType)) {
      return false;
    }
    var attrs = {
      value: value
    ...
    if ($cursor) {
      if (!attrs.value) {
        ...
        var tr1 = state.tr.removeStoredMark(markType);
  ...
  function getFontFamilyMarkDef(options) {
    return {
      ...
      parseDOM: [{
        style: "font-family",
        getAttrs: function getAttrs(fontName) {
          if (!fontName) {
            return false;
          }
  ...
  function addFontFamilyMark(marks, options) {
    return marks.append({
      fontFamily: getFontFamilyMarkDef(options)
    });
  }
  function setFontFamily(options, view, value) {
    if (!value) {
      value = "";
    } else {
      var defaultFontFamily = options.default.toLowerCase();
  ...
  function getFontSizeMarkDef(options) {
    return {
      ...
      parseDOM: [{
        style: "font-size",
        getAttrs: function getAttrs(sizeValue) {
          if (!sizeValue) {
            return false;
          }
  ...
  function addFontSizeMark(marks, options) {
    return marks.append({
      fontSize: getFontSizeMarkDef(options)
    });
  }
  function setFontSize(options, view, value) {
    if (!value) {
      value = "";
    } else {
  ...
  function getFontColorMarkDef(options) {
    return {
      ...
      parseDOM: [{
        style: "color",
        getAttrs: function getAttrs(colorValue) {
          if (!colorValue) {
            return false;
  ...
  function addFontColorMark(marks, options) {
    return marks.append({
      fontColor: getFontColorMarkDef(options)
    });
  }
  function setFontColor(options, view, value) {
    if (!value) {
      value = "";
    } else {
      if (value === options.default) {
  ...
  function addBackgroundColorMark(marks, options) {
    return marks.append({
      backgroundColor: getBackgroundColorMarkDef(options)
    });
  }
  function setBackgroundColor(options, view, value) {
    if (!value) {
      value = "";
    } else {
      if (value === options.default) {
  ...
  function getScriptMarkDef() {
    return {
      ...
      parseDOM: [{
        ...
        getAttrs: function getAttrs(alignmentType) {
          if (!alignmentType) {
            return false;
            ...
            //if some other alignmenType comes such as "vertical-align:baseline" ignore it.
            return false;
          }
        }
      }, {
  ...
  function addScriptMark(marks, options) {
    return marks.append({
      script: getScriptMarkDef()
    });
  }
  function toggleSuperScript(view) {
    return applyScript("sup", view);
  }
  function toggleSubScript(view) {
    return applyScript("sub", view);
  }
  function markApplies$3(doc, ranges, type) {
    var rangeAllowsMark = function rangeAllowsMark(i) {
      var ref = ranges[i];
      var $from = ref.$from;
      var $to = ref.$to;
  ...
  function applyScript(type, view) {
    if (!type) {
      return;
    }
    var state = view.state;
    ...
    var $cursor = ref.$cursor;
    var ranges = ref.ranges;
    if (empty && !$cursor || !markApplies$3(state.doc, ranges, markType)) {
      return false;
    }
    var attrs = {};
    attrs.type = type;
     ...
     * Case 1: If $from === $to then we would have value for $cursor
     *
     *      b. Sub-Case 1:If script mark with the attrs value "super" is already present , and if we want to change the script type to "sub",
     *      then we need to first remove the script mark with the attrs value "super" and then only add a new script mark with the attrs set to "sub".
     *
    ...
    if (dispatch) {
      if ($cursor) {
        if (markType.isInSet(state.storedMarks || $cursor.marks())) {
          var mark = markType.isInSet(state.storedMarks || $cursor.marks());
  ...
  function addInlineCodeMark(marks, options) {
    return marks.append({
      inlineCode: getInlineCodeMarkDef()
    });
  }
  function toggleInlineCode(view) {
    return toggleMark$1(view.state.schema.marks.inlineCode)(view.state, view.dispatch);
  }

  var featureSpec = {
    bold: {
      addMarks: function addMarks(marks, options) {
        marks = marks.remove("strong"); // remove bold mark if present already
        return addBoldMark(marks);
      },
      addCommands: function addCommands(view, options) {
        view.commands.toggleBold = toggleBold.bind(null, view); // view.commands.toggleBold()
      },
      registerShortcut: function registerShortcut(view, options) {
    ...
    strikethrough: {
      addMarks: function addMarks(marks, options) {
        return addStrikethroughMark(marks);
      },
      addCommands: function addCommands(view, options) {
    ...
    inlineCode: {
      addMarks: function addMarks(marks, options) {
        ...
        return addInlineCodeMark(marks);
      },
      addCommands: function addCommands(view, options) {
  ...
  var mac$1$1 = typeof navigator != "undefined" && /Mac/.test(navigator.platform);
  ...
  function keyName$1(event) {
    ...
    var ignoreKey = mac$1$1 && event.metaKey && event.shiftKey && !event.ctrlKey && !event.altKey || ie$1 && event.shiftKey && event.key && event.key.length == 1 || event.key == "Unidentified";
  ...
  var mac$4 = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : false;
  var shortcutsPluginName = "shortcuts";
  ...
  function modifiers$1(name, event) {
  ...
  function getShortcutsPlugin() {
    return new Plugin({
      key: new PluginKey(shortcutsPluginName),
      state: {
        init: function init() {
          return {};
        },
        apply: function apply(tr, state) {
          if (tr.meta && tr.meta[shortcutsPluginName]) {
            var shortcutObj = tr.meta[shortcutsPluginName];
            for (var key in shortcutObj) {
      ...
      props: {
        handleKeyDown: function handleKeyDown(view, event) {
          var map = normalize$1(this.getState(view.state));
          var name = keyName$1(event),
            baseName;
          var direct = map[modifiers$1(name, event)];
  ...
  function registerShortcut(view, keyCombination, functionToExecute) {
    var metaObj = {};
    metaObj[keyCombination] = functionToExecute;
    var tr = view.state.tr;
    view.dispatch(tr.setMeta(shortcutsPluginName, metaObj));
  }
  ...
  function getCursorEventsPlugin() {
    // a plugin that throws the marks at current cursor position whenever view is updated,
  ...
  var getMarks = function getMarks(view) {
  ...
  var sampleConfig = ["bold", "italics", "underline", "strikethrough", {
    name: "link",
    regex: defaultLinkRegex
  }, {
    name: "fontSize",
    default: "10pt"
  }, {
  ...
  function preProcessConfig(config) {
    return config.map(function (subConfig) {
      if (typeof subConfig === "string") {
        return getSubConfig(subConfig, sampleConfig);
      } else {
        var defaultSubConfig = getSubConfig(subConfig.name, sampleConfig);
        return Object.assign({}, defaultSubConfig, subConfig);
      }
    });
  }
  function getSubConfig(name, config) {
    var subConfigFeature = config.filter(function (subConfig) {
    ...
    return typeof subConfigFeature === "string" ? {
      name: subConfigFeature
    } : subConfigFeature;
  }
  function addMarks(marks, config) {
    config = preProcessConfig(config);
  ...
  function addShortcuts(view, config) {
    config = preProcessConfig(config);
  ...
  function addCommands(view, config) {
    config = preProcessConfig(config);
    view.commands = {};
    config.forEach(function (subConfig) {
  ...
  const historyKey = new PluginKey$1("history");
  const closeHistoryKey = new PluginKey$1("closeHistory");
  /**
  ...
  */
  const joinBackward = (state, dispatch, view) => {
      ...
      // If there is no node before this, try to lift
      if (!$cut) {
          let range = $cursor.blockRange(), target = range && liftTarget$1(range);
          if (target == null)
              return false;
          if (dispatch)
      ...
      if ($cursor.parent.content.size == 0 &&
          ...
          for (let depth = $cursor.depth;; depth--) {
              let delStep = replaceStep$1(state.doc, $cursor.before(depth), $cursor.after(depth), Slice$1.empty);
              if (delStep && delStep.slice.size < delStep.to - delStep.from) {
                  if (dispatch) {
  ...
  */
  const joinForward = (state, dispatch, view) => {
      ...
      if ($cursor.parent.content.size == 0 &&
          ...
          let delStep = replaceStep$1(state.doc, $cursor.before(), $cursor.after(), Slice$1.empty);
          if (delStep && delStep.slice.size < delStep.to - delStep.from) {
              if (dispatch) {
                  let tr = state.tr.step(delStep);
  ...
  */
  const joinUp = (state, dispatch) => {
      ...
      if (nodeSel) {
          if (sel.node.isTextblock || !canJoin$1(state.doc, sel.from))
              return false;
          point = sel.from;
      }
      else {
  ...
  */
  const joinDown = (state, dispatch) => {
      ...
      if (sel instanceof NodeSelection$1) {
          if (sel.node.isTextblock || !canJoin$1(state.doc, sel.to))
              return false;
          point = sel.to;
      }
      else {
  ...
  */
  const lift = (state, dispatch) => {
      let { $from, $to } = state.selection;
      let range = $from.blockRange($to), target = range && liftTarget$1(range);
      if (target == null)
          return false;
      if (dispatch)
          dispatch(state.tr.lift(range, target).scrollIntoView());
  ...
  */
  const liftEmptyBlock = (state, dispatch) => {
      ...
      if ($cursor.depth > 1 && $cursor.after() != $cursor.end(-1)) {
          ...
          if (canSplit$1(state.doc, before)) {
              if (dispatch)
  ...
  function splitBlockAs(splitNode) {
      return (state, dispatch) => {
          ...
          if (state.selection instanceof NodeSelection$1 && state.selection.node.isBlock) {
              if (!$from.parentOffset || !canSplit$1(state.doc, $from.pos))
                  return false;
              if (dispatch)
          ...
          if (dispatch) {
              ...
              if (!types && !can && canSplit$1(tr.doc, tr.mapping.map($from.pos), 1, deflt ? [{ type: deflt }] : undefined)) {
                  if (deflt)
                      types = [{ type: deflt }];
                  can = true;
              }
  ...
  */
  const selectAll = (state, dispatch) => {
      if (dispatch)
          dispatch(state.tr.setSelection(new AllSelection$1(state.doc)));
      return true;
  };
  function joinMaybeClear(state, $pos, dispatch) {
      ...
      if (!$pos.parent.canReplace(index, index + 1) || !(after.isTextblock || canJoin$1(state.doc, $pos.pos)))
          return false;
      if (dispatch)
          dispatch(state.tr
  ...
  function deleteBarrier(state, $cut, dispatch, dir) {
      ...
      let range = selAfter && selAfter.$from.blockRange(selAfter.$to), target = range && liftTarget$1(range);
      if (target != null && target >= $cut.depth) {
          if (dispatch)
              dispatch(state.tr.lift(range, target).scrollIntoView());
          return true;
  ...
  */
  function wrappingInputRule(regexp, nodeType, getAttrs = null, joinPredicate) {
      return new InputRule(regexp, (state, match, start, end) => {
          ...
          if (before && before.type == nodeType && canJoin$1(tr.doc, start - 1) &&
  ...
  */
  ...
  function getDefaultPlugins(schema) {
      let plugins = [
